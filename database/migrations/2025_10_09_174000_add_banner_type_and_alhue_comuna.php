...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('tarjetas', function (Blueprint $table) {
            ...
            $table->enum('diseno_tarjeta', $disenosPermitidos)
                  ...
                  ->comment('Updated ENUM list for card designs')
                  ->change();

            $table->enum('tipo_contenido', $contenidosPermitidos)
                  ->nullable()
                  ->comment('Updated ENUM list for content types')
                  ->change();
        });
    }
    ...
    {
        Schema::table('tarjetas', function (Blueprint $table) {
            $disenosAntiguos = ['tarjetaBasica', 'tarjetaMediana', 'tarjetaGrande', 'infoAyuda', 'tarjetasYoutube'];
            $contenidosAntiguos = ['webView', 'listadoTarjetas', 'videosYoutube', 'pdf', 'subTitulo', 'infoAyudaContenido'];
